...
void TransactionHelper::LoadAccounts(string file_name) {
  ...
  while (!infile.eof()) {
    ...
    cout << "[From string]" << acc_holder << "." << endl; // this words
    ...
    cout << "[From user object]" << acc_holder << "." << endl; // doesnt work!!! wtf
    // u.SetName("insert name here"); // so, this also works...
    u.SetNum(acc_num);
  ...
  for (int i = 0; i < users.size(); i++) {
    if (((users.at(i).GetName()).compare("END_OF_FILE         ")) == 0) {
      users.erase(users.begin()+i); // Get rid of END_OF_FILE user
    }
  }
...
void TransactionHelper::PrintWelcomeMessage() {
  ...
  cout << " _   _                 _____        ____ _ _   " << endl;
  cout << "| | | | _____      __ |_   _|__    / ___(_) |_ " << endl;
  cout << "| |_| |/ _ \\ \\ /\\ / /   | |/ _ \\  | |  _| | __|" << endl;
  cout << "|  _  | (_) \\ V  V /    | | (_) | | |_| | | |_ " << endl;
  cout << "|_| |_|\\___/ \\_/\\_/     |_|\\___/   \\____|_|\\__|" << endl;

  cout << "\nWelcome to How To Git Banking System." << endl;
